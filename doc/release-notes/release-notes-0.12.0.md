Worldcoin Core version 0.12.0 is now available from:

  <https://worldcoin.org/bin/worldcoin-core-0.12.0/>
...
pre-0.12 versions of Worldcoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Worldcoin Core version 0.11 or
earlier.
...
[libsecp256k1](https://github.com/worldcoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/worldcoin/bips/blob/master/bip-0130.mediawiki)
...
Worldcoin Core 0.12 will have a strict maximum size on the mempool. The
...
Worldcoin Core 0.12 also introduces new default policy limits on the length and
...
Worldcoin Core 0.12 nodes. Worldcoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/worldcoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Worldcoin Core 0.12 does not yet have support for
...
In Worldcoin Core 0.12, when mempool limit has been reached a higher minimum
...
notes of v0.11.0](https://github.com/worldcoin/worldcoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/worldcoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/worldcoin/bips/blob/master/bip-0065.mediawiki)
...
- #6214 `87406aa` [OSX] revert renaming of Worldcoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new worldcoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include worldcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix worldcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of worldcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override WORLDCOIND and WORLDCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [worldcoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add WORLDCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libworldcoinconsensus (Braydon Fuller)
