Worldcoin Core version 0.11.0 is now available from:

  <https://worldcoin.org/bin/worldcoin-core-0.11.0/>
...
For example, add the following to `worldcoin.conf`:
...
related to the blockchain in the worldcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Worldcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Worldcoin Core will 
...
For an example script see [worldcoin-submittx](https://github.com/laanwj/worldcoin-submittx).
...
- #5334 `cf87536` libworldcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new worldcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only WorldcoinAmountField
- #5753 `0f44672` Add worldcoin logo to about screen
...
- #5809 `46bfbe7` Add worldcoin-cli man page
...
- #5849 `41259ca` contrib/init/worldcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Worldcoin-Qt.app to Worldcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for worldcoin-tx)
