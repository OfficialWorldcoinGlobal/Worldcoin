...
ClientModel is used by the main application `worldcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/worldcoin/worldcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a worldcoin:... link from a browser).
...
* UI elements like WorldcoinAmountField, which inherit from QWidget.
* `worldcoinstrings.cpp`: automatically generated
* `worldcoinunits.(h/cpp)`: WDC / mWDC / etc handling
* `callback.h`
...
* don't change `local/worldcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
