...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(WORLDCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // worldcoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseWorldcoinURI(arg, &r) && !r.address.isEmpty())
            {
...
//
// OSX-specific way of handling worldcoin: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in worldcoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(WORLDCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // worldcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseWorldcoinURI(s, &recipient))
            {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom worldcoin addresses are not supported
        ...
        // Worldcoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(WorldcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
